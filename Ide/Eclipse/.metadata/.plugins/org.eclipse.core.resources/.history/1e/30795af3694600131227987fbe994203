...
{	
	...
	this ->x = 0;
	this ->y = 0;
	this ->width = Environment::GetInstance()->GetScreenWidth();
	this ->height = Environment::GetInstance()->GetScreenHeight() - guidefPanel->GetAttributeInt("heightdiff");
...
void BoardView::BoardInitialized()
{
	...
	imgTex = new Texture(rowSize, columnSize, 32, ((UserImage*)userImg)->GetColors());
	...
	refTex = new Texture(refImage->GetWidth(), refImage->GetHeight(), 32, refImage->GetColors());
	refTex->BindTexture();
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
